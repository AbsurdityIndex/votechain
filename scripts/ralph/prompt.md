...
5. Use `RALPH_BASE_URL` as the canonical app host for browser flows (default: `http://127.0.0.1:4321/votechain/poc/`).
...
6. For UI/UX changes, run Playwright flow verification.
   - Target host: `RALPH_BASE_URL`.
   - If host is unavailable, start local dev server at `127.0.0.1:4321` and continue.
